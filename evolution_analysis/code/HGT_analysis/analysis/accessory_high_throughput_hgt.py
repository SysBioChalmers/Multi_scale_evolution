...
if __name__== "__main__":
    ...
    with open('./accessory_OG.csv', 'r') as file :
        lines = file.readlines()[1:]

    panids = [line.strip() for line in lines]
    print(len(panids))  #2553
    # print(panids[:3])

    for panid in panids :
        if '@' in panid :
            index = SeqIdIndex[panid]
            ortholog = IndexOrtholog[index]
            ortholog_panid[ortholog] = panid
        else :
            newpanid = 'Saccharomyces_cerevisiae@'+panid
            index = SeqIdIndex[newpanid]            
            ortholog = IndexOrtholog[index]
            ortholog_panid[ortholog] = panid
    ...
    outfile = open("./accessory_new_HGT.tsv", "wt")
    ...
    e_minus = 1e-200
    for ortholog in list(ortholog_panid.keys()) :
        print(ortholog)
